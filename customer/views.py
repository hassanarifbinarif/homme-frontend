...
def serviceWorkerFirebase(request):
    data='importScripts("https://www.gstatic.com/firebasejs/8.6.3/firebase-app.js");' \
         ...
         'dbrequest.onsuccess = function(event) { dbInSW = event.target.result; };'\
         'function updateCount(newCount) {const transaction = dbInSW.transaction([storeNameInSW], "readwrite");const objectStore = transaction.objectStore(storeNameInSW);const updateRequest = objectStore.put({ id: 1, count: newCount });}'\
         'self.addEventListener("notificationclick", (event) => {'\
         '    const clickedNotification = event.notification;'\
         '    let badgeCount = JSON.parse(event.notification.data["FCM_MSG"]["data"]["data"]);'\
         '    updateCount(badgeCount.badge);'\
         '    console.log("notification hahaha", event.notification);'\
         '    /*const promiseChain = updateCount();*/'\
